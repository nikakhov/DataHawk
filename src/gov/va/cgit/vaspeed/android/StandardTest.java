...
import gov.va.cgit.vaspeed.android.Constants;
import gov.va.cgit.vaspeed.android.SaveResults;
import gov.va.cgit.vaspeed.android.AndroidUiServices.TextOutputAdapter;
import gov.va.cgit.vaspeed.android.Calspeed.LatLong;
...
public class StandardTest implements Runnable {
	...
	public void run() {
		try {
			...
			if (runStatus == RUNCOMMAND_INTERRUPT){
				...
				//Upload to our PostreSQL server
				//Get values to upload
				String up = ((TextView) ((Activity) Calspeed.getContext()).findViewById(R.id.uploadSpeed)).getText().toString();
				String down = ((TextView) ((Activity) Calspeed.getContext()).findViewById(R.id.downloadSpeed)).getText().toString();
				String delay = ((TextView) ((Activity) Calspeed.getContext()).findViewById(R.id.latencySpeed)).getText().toString();
				String jitter = ((TextView) ((Activity) Calspeed.getContext()).findViewById(R.id.jitterSpeed)).getText().toString();
				
				//Indoor or outdoor
				String door = ((ToggleButton) ((Activity) Calspeed.getContext()).findViewById(R.id.indoorOutdoorToggle)).getText().toString();
				Log.v("abc", testLatLong.Longitude.toString());
				if (testLatLong.Longitude==0 || testLatLong.Latitude==0){
					double[] mylocation=getGPS();
					testLatLong.Latitude=mylocation[0];
					testLatLong.Longitude=mylocation[1];
					Log.v("inc", Double.toString(mylocation[1]));
				}
				
				//Send get request
				try {
				    HttpClient client = new DefaultHttpClient(); 
				    String getURL = Constants.fillAPIPath + "/fill?lat=" +  testLatLong.Latitude + "&lon=" + testLatLong.Longitude
				    		+ "&up=" + up + "&down=" + down + "&delay=" + delay + "&jitter=" + jitter + "&door='" + URLEncoder.encode(door, "UTF-8") +
				    		"'&networkType='" +  URLEncoder.encode(networkType, "UTF-8") + "'&date='" + URLEncoder.encode(date.toString(), "UTF-8") + "'";
	...
	private double[] getGPS() {
        ...
        Log.v("provs", lm.getProviders(true).toString());
	...
	private void resetResults(){
		...
		uiServices.setResults(Constants.THREAD_WRITE_JITTER_DATA,context.getResources().getString(R.string.jitterLabel), "0", false, false);
	}
	...
	private void setUDPJitterFinal(ProcessIperf udp1,ProcessIperf udp2){
		...
		String udpMessage = context.getResources().getString(R.string.jitterLabel);
